  ...
  return Object.assign(class JsonApiSerializer extends Serializer {
    ...
    processResponse (contextResponse, request, response) {
      ...
      if (payload && payload.records) {
        contextResponse = this.showResponse(contextResponse,request, payload.records, payload.include)
      }
    ...
    showResponse (contextResponse, request, records, include) {
      ...
      if (!ids && method === methods.find) {
        ...
        //if (count > limit) {
        if(query && query[pageOffset] && query[pageLimit]) {
          let queryLength = 0
          ...
          Object.assign(output[reservedKeys.links], {
            ...
            [reservedKeys.last]: `${paged}${queryLength ? '&' : '?'}` +
              `${encodedOffset}=${Math.floor(2000 / limit)}` +
              `&${encodedLimit}=${limit}`,
            [reservedKeys.next]: (offset + limit) < 2000 ? `${paged}${queryLength ? '&' : '?'}` +
            `${encodedOffset}=${(offset + limit)}&${encodedLimit}=${limit}` : null,
            [reservedKeys.prev]: offset == 0 ?`${paged}${queryLength ? '&' : '?'}` +
              `${encodedOffset}=${(offset - limit)}&${encodedLimit}=${limit}` : null
          })
        }
      }
      if (records.length) {
        ...
        output[reservedKeys.primary] = recordTypes[originalType][relatedField][keys.isArray] ? [] : null
      }
      // Set related records.
      if (relatedField) {
        output[reservedKeys.links] = {
