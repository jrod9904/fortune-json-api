...
function mapRecord (type, record) {
  ...
  for (let i = 0, j = unionFields.length; i < j; i++) {
    ...
    if (!fieldDefinition || fieldDefinition[keys.type]) {
      ...
      //remove salesforce record attributes
      if(value && value.attributes) delete value.attributes;
      for (let i in value) {
        //delete attributes from children relationships
        if (value[i] && value[i].attributes) {
          delete value[i].attributes;
          for (let j in value[i]) {
            if (value[i][j] && value[i][j].attributes) {
              delete value[i][j].attributes;
              for (let k in value[i][j]) {
                if (value[i][j][k] && value[i][j][k].attributes) {
                  delete value[i][j][k].attributes;
                  for (let l in value[i][j][k]) {
                    if (value[i][j][k][l] && value[i][j][k][l].attributes) {
                      delete value[i][j][k][l].attributes;
                      for (let m in value[i][j][k][l]) {
                        if (value[i][j][k][l][m] && value[i][j][k][l][m].attributes) {
                          delete value[i][j][k][l][m].attributes;
                        }
                      }
                    }
                  }
      ...
      // else 
      clone[reservedKeys.attributes][field] = value
    ...
    const linkedType = inflectType[fieldDefinition[keys.link]] ?
      ...
      //only show self link if ID is present
      // if(clone && clone[reservedKeys.attributes] && clone[reservedKeys.attributes][field] && clone[reservedKeys.attributes][field].id) {
        clone[reservedKeys.relationships][field] = {
          [reservedKeys.links]: {
            // [reservedKeys.self]: prefix + uriTemplate.fillFromObject({
            //   type: field ?
            //   field : type,
            //   ids: clone && clone[reservedKeys.attributes] && clone[reservedKeys.attributes][field] && clone[reservedKeys.attributes][field].id? clone[reservedKeys.attributes][field].id : 'zzz'
            // })
            //orginal self relationship links
            [reservedKeys.self]: prefix + uriTemplate.fillFromObject({
              type: inflectType[type] ?
                inflection.transform(type, typeInflections[1]) : type,
              ids: id,
  ...
  for (const parameter of Object.keys(query)) {
    ...
    if (parameter.match(isFilter)) {
      ...
      if(isRelationFilter(field)){
        const isValidPath = isValidRelationPath(recordTypes, fields, field.split('.'))
        if (!isValidPath) {
          throw new BadRequestError(`Path ${field} is not valid`)
        }
      } else if (!(field in fields)) throw new BadRequestError(
        `The field "${field}" is non-existent.`)

      const fieldType = getLastTypeInPath(recordTypes, fields,field.split('.'));
...
function isValidRelationPath (recordTypes,fieldsCurrentType,remainingPathSegments ) {
  if (!remainingPathSegments.length ) return false

  const pathSegment = remainingPathSegments[0]

  if (!fieldsCurrentType[pathSegment]) return false
  if ( fieldsCurrentType[pathSegment].type && remainingPathSegments.length === 1 ) return true
...
function getLastTypeInPath ( recordTypes,fieldsCurrentType,remainingPathSegments ) {
  ...
  if ( !fieldsCurrentType[pathSegment] ) return {}
